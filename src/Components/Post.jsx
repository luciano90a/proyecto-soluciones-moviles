...
const Post = ({post}) => {
  ...
  const {like_post, getLikedPosts, likedPosts, user, dislike_post, getPosts} =
    ...
    <View style={styles.postContainer}>
      ...
      <View style={styles.postFooter}>
        <TouchableOpacity onPress={isLiked ? handleDislike : got_like}>
          <Icon
            name={isLiked ? 'check-circle' : 'check-circle-outline'}
            color="black"
            size={30}
          />
        </TouchableOpacity>
      ...
      {showLikedMessage && (
        <Text style={styles.likeMessage}>¡Ya le has dado like!</Text>
      )}
      {!showLikedMessage && !isLiked && (
        <Text style={styles.notLikedMessage}>Aún sin dar like</Text>
      )}
